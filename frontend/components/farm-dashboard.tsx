...
export function FarmDashboard() {
  ...
  const {
    ...
    queryFn: () =>
      ensureDefaultFarm(session!.user.id, session!.user.email ?? undefined),
    enabled: !!session,
    retry: false,
  });
  ...
  async function handleSeasonChange(
    fieldId: number,
    params: { primaryCrop?: string; plantingDate?: string },
  ) {
    if (!farm) return;
    try {
      ...
      console.error(
        "Couldn't save this field's season details:",
        farmErrorMessage(error),
      );
    }
  }
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      ...
      <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-12 px-4 py-8 sm:px-6">
        {farmErrored && (
        ...
        <section className="space-y-5">
          <SectionHeader
            title="Your fields"
            subtitle="Draw a field to get its yield estimate, growth stage and crop dates."
            action={
              !adding && !farmErrored ? (
                <button
                  type="button"
          ...
          {!fieldsLoading && !fields?.length && !adding && (
            <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-accent/40 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No fields yet -- add your first one to get predictions scoped to
                your own land instead of your county average.
              </p>
            </div>
          )}
          ...
          {!!fields?.length && (
            <ul className="space-y-4">
              {fields.map((field) => (
                <li
                  key={field.id}
                  className="rounded-xl border border-accent/40 bg-card px-4 py-3 shadow-sm"
                >
                  ...
                  {!!crops?.length && (
                    <FieldPredictionPanel
                      field={field}
                      crops={crops}
                      onSeasonChange={(params) =>
                        handleSeasonChange(field.id, params)
                      }
                    />
                  )}
                </li>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="ml-auto flex gap-2">
                  ...
                  <button
                    ...
                    onClick={handleSave}
                    disabled={
                      draftPoints.length < 3 || !fieldName.trim() || saving
                    }
        ...
        {farm && herdsError && (
          <div className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-sm text-destructive">
            <p className="font-medium">
              Livestock and calendar aren&apos;t set up yet.
            </p>
            <p className="mt-1 text-destructive/80">
